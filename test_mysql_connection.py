...
from dotenv import load_dotenv        # loads the .env file so os.getenv can see the server credentials
...
def connect_to_database():
    ...
    try:
        ...
        connection = mysql.connector.connect(
            host=os.getenv("MYSQL_HOST"),                  # the server address       # the port, turned into a number
            user=os.getenv("MYSQL_USER"),                  # the username
            password=os.getenv("MYSQL_PASSWORD"),          # the password
            database=os.getenv("MYSQL_DATABASE"), 
            port=int(os.getenv("MYSQL_PORT"))             # which database to use
        )

        # # is_connected() confirms the connection actually opened successfully.
        # if connection.is_connected():
        #     db_info = connection.server_info()         # ask the server for its version text
